...
type BookInput = Omit<StoredBook, 'id' | 'order'>;
...
export async function fetchHardcoverBooks(rawKey: string): Promise<BookInput[]> {
  ...
  if (data.errors?.length) throw new Error('Invalid API key or no books found');

  const me = data?.data?.me;
  if (!Array.isArray(me) || me.length === 0) throw new Error('Invalid API key or no books found');

  const userBooks: any[] = me[0]?.user_books ?? [];

  return userBooks.map((ub: any) => {
